...
class HabitatDataset(Dataset):
    ...
    def __init__(self, config:dict, user_config:dict, scene_id:str, remark:str, seed_id:int=1) -> None:
        ...
        results_dir_name = time.strftime('%Y-%m-%d_%H-%M-%S') + f'_{dataset_format.value}_{self._scene_id}_seed_{self._seed_id}'
    ...
    def setup(self) -> Dict[str, Union[float, int, str, np.ndarray]]:
        ...
        self._sim.seed(self._seed_id)
        if self._sim.pathfinder.is_loaded:
            rng = np.random.default_rng(self._seed_id)
            initial_state = AgentState()
            initial_state.position = self._sim.pathfinder.get_random_navigable_point()
            yaw = float(rng.uniform(0.0, 2.0 * np.pi))
            initial_state.rotation = quaternion.from_rotation_vector(np.array([0.0, yaw, 0.0], dtype=np.float32))
            self._sim.initialize_agent(0, initial_state)
...
def get_dataset(config:dict, user_config:dict, scene_id:str, remark:str='NONE', seed_id:int=1) -> Union[HabitatDataset]:
    dataset_format = DatasetFormats(config['dataset']['format'])
    if scene_id != 'Eval':
    ...
    if dataset_format in [DatasetFormats.GIBSON, DatasetFormats.MP3D, DatasetFormats.REPLICA]:
        return HabitatDataset(config, user_config, scene_id, remark, seed_id)
    else:
